...
namespace cds_static{
	...
	uint factorization_var::getSize(){
		...
		mem += sizeof(uint)*nLevels; //iniLevel
		mem += sizeof(ushort)*nLevels; //base
		mem += sizeof(ushort)*nLevels; //base_bits
		mem += sizeof(uint)*tamtablebase; //tablebase
		mem += sizeof(uint)*(nLevels+1); //levelsIndex 
		mem += sizeof(uint)*(tamCode/W+1); //levels
		mem += bS->getSize();   
